/* Trigger for Work Process 8 */
trigger trg_WorkProcess8 on Project_Status_Control__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess8Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 8];
        
        if(rtName == 'Step 8 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 8 : After Submit') {
