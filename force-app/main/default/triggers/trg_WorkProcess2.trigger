/* Trigger for Work Process 2 */
trigger trg_WorkProcess2 on Project_Organization__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess2Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 2];
        
        if(rtName == 'Step 2 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 2 : After Submit') {
        ...
        if(submitStatusNew == 'Pending') {
            ...
            CPEM_SendEmailController.SendApprovedEmailToRequester(trigger.new[0].Id, approveUserId, projectManagerId, requesterId, projectName, WP.Work_Process_No__c, WP.name);
        }
        else if(submitStatusOld == 'Pending' && submitStatusNew == 'Rejected') {
            //Send Email to Requester if Rejeced
            CPEM_SendEmailController.SendRejectedEmailToRequester(trigger.new[0].Id, approveUserId, projectManagerId, requesterId, projectName, WP.Work_Process_No__c, WP.name);
        }
