/* Trigger for Work Process 19 */
trigger trg_WorkProcess19 on CCE_Approval_Process_19__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess19Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 19];

        if(rtName == 'Step 19 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 19 : After Submit') {
