/* Trigger for Work Process 15 */
trigger trg_WorkProcess15 on Design_Safety_Standard__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess15Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 15];
        
        if(rtName == 'Step 15 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 15 : After Submit') {
