/* Trigger for Work Process 24 */
trigger trg_WorkProcess24 on Construction_Management__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess24Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 24];

        if(rtName == 'Step 24 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 24 : After Submit') {
