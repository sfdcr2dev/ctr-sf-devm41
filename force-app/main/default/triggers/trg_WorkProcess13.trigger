/* Trigger for Work Process 13 */
trigger trg_WorkProcess13 on Deliverable_List__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess13Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 13];

        if(rtName == 'Step 13 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 13 : After Submit') {
