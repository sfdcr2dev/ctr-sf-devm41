/* Trigger for Work Process 29 */
trigger trg_WorkProcess29 on MCA__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess29Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 29];
      
        if(rtName == 'Step 29 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step  : After Submit') {
