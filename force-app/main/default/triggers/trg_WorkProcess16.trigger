/* Trigger for Work Process 16 */
trigger trg_WorkProcess16 on Cost_Estimate__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess16Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 16];
        
        if(rtName == 'Step 16 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 16 : After Submit') {
