/* Trigger for Work Process 23 */
trigger trg_WorkProcess23 on Surplus_Spar_part_Management__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess23Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 23];

        if(rtName == 'Step 23 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 23 : After Submit') {
