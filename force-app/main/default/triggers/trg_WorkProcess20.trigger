/* Trigger for Work Process 20 */
trigger trg_WorkProcess20 on Project_Procurement_Contract_Mgmt__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess20Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 20];

        if(rtName == 'Step 20 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 20 : After Submit') {
