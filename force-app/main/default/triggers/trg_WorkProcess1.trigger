/* Trigger for Work Process 1 */
trigger trg_WorkProcess1 on Project_Charter__c (before update, after update) {

    if(Trigger.IsBefore && Trigger.IsUpdate) {
    
        CPEM_AllWorkProcessForTriggerController.UpdateActualStartDateWP1(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        string currentStatus = trigger.new[0].Work_Process_Status__c;
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 1];
        
        if(rtName == 'Step 1 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 1 : After Submit') {
