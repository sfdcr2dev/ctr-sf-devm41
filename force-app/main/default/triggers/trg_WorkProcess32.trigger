/* Trigger for Work Process 32 */
trigger trg_WorkProcess32 on CR_Completion_Report__c (before update, after update) {

    if(Trigger.IsBefore && Trigger.IsUpdate) {
    
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess32Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
        CPEM_AllWorkProcessForTriggerController.UpdateActualEndDateWP1(Trigger.New[0]);        
        CPEM_AllWorkProcessForTriggerController.UpdateProjectCompletedStatus(Trigger.New[0]);     
    }
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 32];
        
        if(rtName == 'Step 32 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 32 : After Submit') {
