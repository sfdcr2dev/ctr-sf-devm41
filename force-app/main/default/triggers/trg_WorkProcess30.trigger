/* Trigger for Work Process 30 */
trigger trg_WorkProcess30 on Close_Out_Report_Lesson_Learnt__c (before update, after update) {
    ...
    if(Trigger.IsBefore && Trigger.IsUpdate) {
        ...
        CPEM_AllWorkProcessForTriggerController.UpdateWorkProcess30Status(Trigger.Old[0].SME__c, Trigger.New[0].SME__c, Trigger.New[0].Project__c, Trigger.New[0]);
    ...
    if(Trigger.IsAfter && Trigger.IsUpdate) {
        ...
        Master_Data_Detail__c WP = [select id, name, Work_Process_No__c from Master_Data_Detail__c where Work_Process_No__c =: 30];
        
        if(rtName == 'Step 30 : Before Submit' && waitingForApprove) {
        ...
        else if(rtName == 'Step 30 : After Submit') {
