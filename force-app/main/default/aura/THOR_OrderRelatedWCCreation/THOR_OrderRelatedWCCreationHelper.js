({
	...
	getOrderDetailById: function (component, event, helper) {
		component.set('v.form.Order__c', component.get('v.orderId'));
		var orderId = component.get('v.orderId');
		// alert(component.get("v.orderId"));
		var action = component.get('c.getOrderDetailById');
		action.setParams({
			orderId: orderId
		});
		...
		action.setCallback(this, function (response) {
			...
			if (state === 'SUCCESS') {
				...
				//if(result.Order__r.Requester__c != null){
				//	// alert(result.Main_Work_Center_Text__c);Main_Work_Center_Text__c
				//	component.set("v.form.Thaioil_Supervisor_Indicator__c", result.Notification__r.Requester__c);
				//}
