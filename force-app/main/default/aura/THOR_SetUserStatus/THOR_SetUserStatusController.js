({
    doInit: function (component, event, helper) {
        ...
        if (target == "notification") {
            helper.getNotificationAvailableStatus(component);
        }
        else {
            helper.getOrderAvailableStatus(component);
        }
    },
...
//    },
    ...
    setUserStatus: function (component, reload) {
        ...
        if (target == "order") {
            ...
            let allStatusList = component.get("v.allStatusList");
            let allStatusListTemp = [];
            allStatusList.forEach(function(value, index, self) {
                allStatusListTemp.push(value.name);
            });
            selectedStatusListString = selectedStatusListString.filter(function(value, index, self) {
                return allStatusListTemp.indexOf(value) < 0;
            });
        }
