({
    ...
    updateFinalRiskAssessment: function (component, event, helper, resolve) {
        ...
        risk.RiskClosedDate__c = $A.localizationService.formatDate(new Date(), 'yyyy-MM-dd');
    ...
    /* For Final Risk Assessment */
    // Require Final Risk Reason when remaining risk is 'M' or 'H'
    requireFinalRiskReason: function(component, event, helper) {
        let riskLevel = component.get('v.risk.FinalRisk__c');
        let reason = component.get('v.risk.FinalRiskReason__c');
        return (riskLevel == 'M' || riskLevel == 'H') && $A.util.isEmpty(reason);
    },
    /**
